...
> **First-time Windows setup:** Run `npm install` and `npx playwright install chromium` before starting.

Add `--headless` flag (or set `HEADLESS=true` env var) if user requests it. **Wait for the `Ready` message before running scripts.**
...
The dev-browser server uses port 9222. If you see connection errors:
