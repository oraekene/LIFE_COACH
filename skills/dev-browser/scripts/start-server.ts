...
function findPackageManager(): { name: string; command: string } | null {
  ...
  for (const manager of managers) {
    try {
      execSync(`${whichCommand} ${manager.name}`, { stdio: "ignore" });
      return manager;
    } catch {
...
function isChromiumInstalled(): boolean {
  ...
  const playwrightCacheDir = isWindows
...
function cleanupStalePort(port: number): void {
  try {
    if (isWindows) {
      ...
      for (const line of lines) {
        ...
        if (pid && pid !== "0") {
          ...
          try {
            ...
            // May fail if process already ended or requires admin
          }
        }
      }
    } else {
      ...
      if (pid) {
        console.log(`Cleaning up stale Chrome process on CDP port ${port} (PID: ${pid})`);
        execSync(`kill -9 ${pid}`);
      }
    }
  } catch {
