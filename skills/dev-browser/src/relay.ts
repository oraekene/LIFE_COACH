...
export async function serveRelay(options: RelayOptions = {}): Promise<RelayServer> {
  ...
  // CDP discovery endpoints for Playwright/Chrome clients
  const handleJsonVersion = (c: any) => {
    log("Handling /json/version request");
    return c.json({
      Browser: "Chrome/Extension-Bridge",
      "Protocol-Version": "1.3",
      "User-Agent": "dev-browser-relay/1.0.0",
      "V8-Version": "V8",
  ...
  const handleJson = (c: any) => {
    log("Handling /json request");
    return c.json(
      Array.from(connectedTargets.values()).map((target) => ({
        ...
        webSocketDebuggerUrl: `ws://${host}:${port}/cdp/${target.sessionId}`,
      }))
    );
  };
